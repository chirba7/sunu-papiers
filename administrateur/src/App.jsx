...
function HouseEditModal({ house, delegates, onClose, onSaved, onError }) {
  ...
  const [saving,setSaving]=useState(false)
  const submit=async(e)=>{e.preventDefault();if(!form.fields.length){onError('Sélectionnez au moins un champ du certificat.');return}setSaving(true);const body=new FormData();['region','departement','commune','quartier','delegateId'].forEach(k=>body.append(k,form[k]));body.append('fields',JSON.stringify(form.fields));if(form.seal)body.append('seal',form.seal);try{await api(`/admin/houses/${house.id}`,{method:'PUT',body});onSaved()}catch(error){onError(error.message)}finally{setSaving(false)}}
