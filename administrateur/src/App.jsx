...
function Toast({ message, onClose }) {
  ...
  // Un échec ne doit pas s'annoncer sous un bandeau « Enregistré » : c'est ce qui
  // a fait passer un refus du storage pour un succès.
  const bad = typeof message === 'object' && message?.tone === 'bad'
  const text = typeof message === 'object' ? message.message : message
  return <div className={`toast ${bad ? 'tone-bad' : ''}`}>{bad ? <X size={20}/> : <CheckCircle2 size={20}/>}<div><strong>{bad ? 'Action impossible' : 'Enregistré'}</strong><span>{text}</span></div></div>
}
...
function DelegateEditModal({ delegate, onClose, onSaved, onError }) {
  ...
  const submit=async(e)=>{e.preventDefault();setSaving(true);try{const updated=await api(`/admin/delegates/${delegate.id}`,{method:'PUT',body:JSON.stringify(form)});onSaved(updated)}catch(error){onError({tone:'bad',message:error.message})}finally{setSaving(false)}}
...
function DelegatesPage() {
  ...
  const submit = async (e) => {
    ...
    try { const created=await api('/admin/delegates',{method:'POST',body:JSON.stringify(form)});setDelegates([created,...delegates]);setForm({firstName:'',lastName:'',email:'',password:randomPassword()});setToast('Le compte délégué a été créé.') } catch(error){setToast({tone:'bad',message:error.message})}
  }
...
// Champs proposés pour le certificat. L'ordre de ce tableau est celui de la mise
// en page : il doit rester aligné sur le catalogue de backend/src/certificate.js.
const CERTIFICATE_FIELDS=[
  {key:'birth_date',label:'Date de naissance'},
  {key:'birth_place',label:'Lieu de naissance'},
  {key:'parents',label:'Noms du père et de la mère'},
  {key:'identity_type',label:'Type de pièce d’identité'},
  {key:'identity_number',label:'Numéro de la pièce'},
  {key:'address',label:'Adresse complète'},
  {key:'resident_since',label:'Année de domiciliation'},
  {key:'lot_number',label:'Numéro de villa / lot'},
]
const DEFAULT_FIELDS=['birth_date','birth_place','identity_type','identity_number','address','resident_since','lot_number']
const houseFields=(house)=>Array.isArray(house?.certificate_fields)&&house.certificate_fields.length?house.certificate_fields:DEFAULT_FIELDS

function FieldPicker({ value, onChange }) {
  const toggle=(key)=>{const next=value.includes(key)?value.filter(k=>k!==key):[...value,key];onChange(CERTIFICATE_FIELDS.map(f=>f.key).filter(k=>next.includes(k)))}
  return <div className="field-picker"><strong>Champs imprimés sur le certificat</strong><p>Le nom du citoyen, la date, le numéro de dossier et la signature sont toujours présents.</p><div className="field-picker-grid">{CERTIFICATE_FIELDS.map(f=><label key={f.key} className={value.includes(f.key)?'on':''}><input type="checkbox" checked={value.includes(f.key)} onChange={()=>toggle(f.key)}/><span>{f.label}</span></label>)}</div></div>
}
